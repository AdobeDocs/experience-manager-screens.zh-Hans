...
通常，在此阶段使用示例或FPO内容。 它可帮助项目经理和关键利益相关者识别UX流中的任何主要差距以及播放和相关触发事件的任何时间问题。此阶段还可以揭示资产定位、规模和大小方面的潜在问题。
...
>[!NOTE]
> 在UX设计阶段，请确认UI设计提取的数据得到了充分支持。例如，并非所有决策者都熟悉其产品数据库的数据结构。 他们可以选择其UX解决方案上不是数据层主字段的描述符。 尽早识别可能需要操纵数据的位置，并将它传送到客户端，可防止以后出现误解。 如果客户端决定在开发开始后扩展或更改UI标签，则需要更加努力来适应这些更改。
