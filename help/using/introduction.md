...
* **交互式体验**：可以使用AEM和SPA编辑器，在Screens渠道中创作并交付支持触屏的Kiosk应用程序。 最佳做法是：应用一致的全渠道设计属性，应用非活动计时器来重置体验，以及应用清晰call to action来执行哪些任务。 登陆页面应当由关键的数字元素组成，这些元素旨在传递价值、吸引用户访问屏幕并提示用户参与。

AEM Screens 提供了一个框架，可以将内容部署到实体设备。 包括媒体内容或触屏应用在内的内容，将会分配给 Screens 中的“渠道”。 在此框架中，AEM Sites应用程序可以通过“渠道”作为内容进行交付。

必须对AEM站点进行格式化，才能将其用于显示设备的尺寸。 应在将其放入Screens中的渠道之前完成此操作。
