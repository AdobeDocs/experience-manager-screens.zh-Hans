...
1. 分配 **接管** 在计划时间用于显示的频道。 然后，从 **渠道分配** 对话框并选择 **保存**：

   * **渠道路径**：单击的路径 **接管** 渠道
   * **优先级**：将此渠道的优先级设置为大于 **MainAdChannel**. 例如，本示例中设置的优先级为8。
   * **受支持的事件**：单击 **空闲屏幕** 和 **计时器**.
   * **计划**：输入您希望此渠道在显示屏上运行的计划文本。 此文本位于 **计划** 本示例中提及的是 *星期三14:00至16:00之间*.
   ...
   * **保持活动状态结束日期**：结束日期和时间。
     ...
     导航到显示区，从 **接管** > **位置** > **主大厅** > **MainLobbyDisplay**，然后单击 **仪表板** ，以便查看已分配渠道及其优先级，如下所示。
