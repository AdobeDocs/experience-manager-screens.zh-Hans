...
***多区域*** 布局允许您创建多个区域内容并使用各种资源，如可组合到单个屏幕中的视频、图像和文本。您可以拖入图像、视频和文本，使其全部融合在一起，创建直观的数字体验。
...
1. **将嵌入式序列（组件）添加到主渠道(MultiZone)**
   ...
   1. 选择渠道路径作为&#x200B;**Zones** —> **渠道** —> **EmbeddedChannels** —> **Zone1**，如下图所示。
...
1. **将渠道分配给显示屏**
   ...
   1. 输入&#x200B;**渠道路径**&#x200B;中&#x200B;**MultiZone**&#x200B;渠道的路径。
   1. 将&#x200B;**支持的事件**&#x200B;设置为&#x200B;**初始负载**、**空闲屏幕**&#x200B;和&#x200B;**计时器**。
   1. 单击&#x200B;**保存**。

      ![图像](/help/user-guide/assets/multi-zone/multizone-img9.png)
   1. 同样，您必须将其他两个嵌入渠道（**Zone1**&#x200B;和&#x200B;**Zone2**）分配给此显示屏。
   1. 将所有三个渠道分配给&#x200B;**休息室**&#x200B;显示屏后，您应能够从显示仪表板视图分配的渠道。
      ...
      >[!IMPORTANT]
      >
      > 一旦将主渠道（本例中为&#x200B;**MultiZone**）分配给显示屏，则必须将另外两个嵌入式渠道&#x200B;**Zone1**&#x200B;和&#x200B;**Zone2**&#x200B;也分配给同一显示屏。
...
1. **注册设备**
   ...
   >[!NOTE]
   >如果尝试视图Screens播放器中的内容，请确保为分配给显示屏的每个渠道单击渠道仪表板中的&#x200B;**更新脱机内容**。
