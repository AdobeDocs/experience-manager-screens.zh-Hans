...
在开始设置数据存储以配置AEM Screens项目的Context Hub配置之前，必须设置Google工作表（用于演示目的）。
...
请按照以下步骤设置一个数据存储区，通过该存储区，您可以使用ContextHub配置和AEM Screens渠道的区段路径。
...
1. 导航到其中一个AEM Screens渠道**。 **以下步骤演示如何使用在AEM Screens渠道中创建的 **DataDrivenRetail** 启用定位。
