...
# 使用AEM Screens {#configuring-adobe-analytics-with-aem-screens}配置Adobe Analytics
...
* **Adobe Analytics与AEM Screens**
* **使用脱机Adobe Analytics发送自定义事件**

## 在Adobe Analytics中与AEM Screens{#sequencing-in-adobe-analytics-with-aem-screens}进行排序

***排序过程***&#x200B;开始，其中包含激活Adobe Analytics服务的数据存储服务。 渠道内容将Adobe Analytics事件与工资单一起发送，即，将数据测试捕获发送到Windows I/O并触发保留事件。 将事件保存到索引数据库中，并进一步放入对象存储中。 根据计划，管理员设置数据，从对象存储中剪切数据，再在块存储中传输数据。 连接时，它会尝试发送最大数量的数据。
...
下表总结了事件的标准数据模型。 它将列表发送到Adobe Analytics的所有字段：
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>字符串</td> 
   <td>timestamp - UTC</td> 
   <td>事件开始日期时间，如果未指定此时间，则事件时间将假定为服务器接收此时间</td> 
  </tr>
  <tr>
   <td> </td> 
   <td>事件时间（结束）</td> 
  ...
  <tr>
   ...
   <td>字符串<br /> </td> 
   <td>UUID</td> 
   <td>标识设备GUID（例如，计算机ID或IP地址的哈希+子网掩码+网络ID +用户代理） — 在此，我们将发送注册时生成的播放器的用户名。</td> 
  </tr>
  <tr>
   <td> </td> 
   <td>计数</td> 
