...
description: 关注此页面，了解渠道分配和DayParting。
...
>[!IMPORTANT]
>
>本节重点介绍AEM 6.5.5 Screens功能包及更高版本的渠道分配和渠道计划。

设置显示后，必须为显示分配渠道以查看您的内容。

本页显示将渠道分配给显示器、了解渠道属性和DayParting。
...
请按照以下部分创建AEM Screens项目并分配渠道以显示内容。
...
1. 从 **设置** 选项，您可以选择渠道 **按路径** 或 **按名称**，输入 **渠道角色**， **优先级**， **受支持的事件**、和 **中断方法**. 此外，您还可以从此对话框启用有趣内容工具提示。
   ...
   >[!NOTE]
   >
   >请参阅 [渠道属性](#channel-properties) 部分，了解有关渠道分配属性的更多信息。
...
此示例展示了在Chrome播放器上的输出。 将渠道分配给显示区后，必须将该设备注册到播放器。
...
将渠道分配给显示并设置重复时间表后，您可以从以下位置查看时间线： **已分配的渠道和计划** 面板。
...
使用周期性计划，您可以为内容设置周期性计划。 单击 **+添加计划** 以向渠道添加周期性计划。

>[!NOTE]
>您可以向渠道添加多个周期性计划。
>周期性时间表引入 *日期分段*，该模式允许您设置一个全局计划，在一天中的特定时间运行多个渠道，并一次性将该设置重复用于所有显示。
...
以下示例在三种不同的场景中解释了渠道中的DayParting：

#### 在划分为多个时隙的一天中播放内容 {#playing-content-on-a-single-day-divided-into-multiple-time-slots}

这个例子说明一家餐厅如何使用DayParting每天展示其早餐、午餐和晚餐菜单。

在本例中，我们将每天划分为不同的时段，以便渠道内容可以在一天中的指定时间播放。 为频道设置周期性计划的以下属性，以根据此用例播放内容。
...
此示例显示了在赌场中实施的DayParting，其中实时活动在每周末的8:00 pm到10:00 pm之间进行，并且在10:00 pm之后到1:00 am之间菜单提供特别优惠。
