...
访问Android的某些特权API，™如 *PowerManager* 或 *Hdmicontrolservices*，签署Android™ `apk` 使用制造商的钥匙。
...
跨Android™监视程序服务通过使用作为Cordova插件实现 *警报管理器*.
...
1. 使用MDM、ADB或Android™ Studio文件资源管理器将此策略JSON文件拖放到 *sdcard* 文件夹(在Android™设备上)。
...
AEM Screens提供远程控制功能。 可在此处详细了解此功能： [Screens远程控制](implementing-remote-control.md)
