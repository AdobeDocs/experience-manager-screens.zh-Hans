...
在的帮助下 ***屏幕启动项***，作者可以预览启动项中的每个渠道，并且应该能够发起审查请求。 审批者组收到通知，可以批准或拒绝请求。 当到达实时日期时，内容将在设备中播放。
...
开始使用之前 *屏幕启动项* 在AEM Screens项目中，确保了解宽限期的概念及其相关性。
...
如果上线日期为 *11月24日，上午9点* 和 *24小时* 是宽限期，则上述操作序列从（起始日期 — 宽限期）开始，即11月23日服务器时间上午9:00。 此设置有24小时来完成上述所有操作，以便内容可到达播放器。 播放器知道此时间段属于启动内容。 因此，内容不会立即播放，但播放器可以将此内容存储为未来版本，并让它在播放器时区的设置实时日期开始播放。
...
>[!NOTE]
>
>开箱即用的Screens Launch的宽限期设置为24小时。 这意味着在为下的资源设置任何启动项的有效日期时， */content/screens*，促销活动将从此偏移开始。
...
1. 右键单击并复制文件。
1. 导航到 `/apps/system/config` 然后右键单击并粘贴。
1. 双击 `/apps/system/config/com.adobe.cq.wcm.launches.impl.LaunchesEventHandler.config` 这样您便可以在CRXDE Lite的编辑器中打开文件。 它必须显示路径的宽限期 */content/screens/* 作为 **86400**. 将该值更改为 **600**.
...
在上例中，您将宽限期设置为10分钟。 因此，在为下的资源设置任何启动项的有效日期时 */content/screens*，促销活动将从此偏移开始。
...
1. 单击 **下一个** 从 **创建启动项** 向导。 此 **包括子页面** 默认情况下选中该选项。
   ...
   要使用 **添加渠道** 选项，导航到要为其创建启动项的渠道并单击 **选择**.
...
1. 输入 **启动项标题** 作为 **SummerPromotions** 而且您无需设置 **启动日期**，如下图所示。 单击&#x200B;**创建**。
   ...
   >
   >
   >*禁用或取消选中* **继承源页面活动数据** 允许复制启动项中没有实时关系的渠道。 因此，如果对原始渠道进行了任何更改，则这些更改不会应用于启动渠道。
...
* **启动日期**  — 实时日期，即根据播放器的时区，内容在Screens播放器中播放的日期或时间。
* **生产就绪**  — 提升后，可以发布渠道并启用现成功能，因此无需进行更改。
* **范围**  — 决定在启动项提升期间提升哪些渠道。
...
创建启动项后，您可以使用向现有启动项添加或删除渠道 **编辑启动项** 选项。

完成后，单击 **保存** 以导航回 **Futurelaunch** 渠道。

### 手动提升屏幕启动项{#promote-the-screens-launch-manually}

您可以使用手动提升启动 **`Promote Launch`** 选项来自 **待处理启动项** 面板。
...
您可以使用删除启动项 **删除启动项** 选项来自 **待处理启动项** 面板。
