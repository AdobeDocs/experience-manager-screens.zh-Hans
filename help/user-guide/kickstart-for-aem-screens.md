...
# 引脚手指南{#kickstart-guide}

AEM Screens的动画入门演示了如何设置和运行AEM Screens项目。 它会指导您设置基本的数字标牌体验，并向每个渠道添加资产和/或视频等内容，并进一步将内容发布到AEM Screens播放器。

>[!NOTE]
>在开始处理项目详细信息之前，请确保您已安装最新的AEM Screens功能包。 您可以使用Adobe ID从[软件分发门户](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)下载最新的功能包。

## 前提条件 {#prerequisites}

请按照以下步骤为AEM Screens创建示例项目并进一步将内容发布到Screens播放器。

>[!NOTE]
>以下教程将在Chrome OS播放器中展示渠道的内容。

>[!IMPORTANT]
>**OSGi配置设置**
>必须启用空推荐人才能允许设备向服务器发布数据。 例如，如果禁用了空的推荐人属性，则设备无法向后发布屏幕截图。 目前，部分这些功能仅在OSGi配置中启用了Apache Sling推荐人过滤器允许空时可用。 仪表板可能会显示警告，指出安全设置可能会阻止某些功能正常工作。
>请按照以下步骤启用&#x200B;***Apache Sling推荐人过滤器允许空***:
...
1. 单击&#x200B;**创建Screens项目**&#x200B;以创建新Screens项目。 输入&#x200B;**DemoScreens**&#x200B;的标题，然后单击&#x200B;**保存**。

   ![图像](assets/kickstart/demo-1.png)

   >[!NOTE]
   >创建项目后，它会将您带回Screens项目主页。 现在，您可以选择自己的项目。在项目中，有5个不同的文件夹，标题为&#x200B;**Applications**、**渠道**、**设备**、**位置**&#x200B;和&#x200B;**计划**。

### 创建渠道 {#creating-channel}

创建AEM Screens项目后，您需要创建一个新渠道来管理内容。

请按照以下步骤为项目创建新渠道:

1. 创建项目后，选择&#x200B;**DemoScreens**&#x200B;项目并选择&#x200B;**渠道**&#x200B;文件夹，如下图所示。 单击操作栏中的&#x200B;**+创建**。
...
1. 将&#x200B;**标题**&#x200B;输入为&#x200B;**TestChannel**，然后单击&#x200B;**创建**。
   ...
   **TestChannel**&#x200B;现已添加到您的渠道文件夹，如下图所示。
...
>[!NOTE]
>***位置***&#x200B;将您的各种数字标牌体验划分为多个区域，并根据不同屏幕的位置包含显示屏的配置。
...
1. 将&#x200B;**Title**&#x200B;输入为&#x200B;**LobbyDisplay**，然后单击&#x200B;**创建**。

   ![图像](assets/kickstart/demo-disp3.png)

   现在，将名为&#x200B;**TestDisplay**&#x200B;的新显示屏添加到您的位置&#x200B;**TestLocation**，如下图所示。
...
1. 从&#x200B;**设置**&#x200B;选项中，选择渠道&#x200B;**按路径**&#x200B;和&#x200B;**支持的事件**&#x200B;作为&#x200B;**初始加载**&#x200B;和&#x200B;**空闲屏幕**。

   >[!NOTE]
   >
   >默认情况下，**渠道角色**、**优先级**&#x200B;和&#x200B;**中断方法**&#x200B;均填充。 请参阅[渠道属性](/help/user-guide/channel-assignment-latest-fp.md#channel-properties)部分，了解有关渠道分配属性的更多信息。
   ...
   >[!NOTE]
   >*循环计划*允许您为渠道设置循环计划。 您为一个渠道设置了多个循环计划。
...
>[!IMPORTANT]
>Chrome OS播放器可以在开发人员模式下作为Chrome Browser插件安装，无需使用实际的Chrome播放器设备。 要进行安装，请执行以下步骤：
>
...
1. 选择显示屏的路径，作为&#x200B;**DemoScreens** —> **位置** —> **TestLocation** —> **LobbyDisplay**，然后单击&#x200B;**分配**。
...
您的渠道中的所有资源现在都在您的Chrome OS播放器上播放。
