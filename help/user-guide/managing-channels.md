...
description: 按照此页面了解如何创建和管理渠道。 它还介绍了渠道功能板和编辑渠道的内容。
...
| 序列渠道 | 允许创建一个按顺序播放组件的渠道（在幻灯片放映中逐一播放）。 |
| 应用程序渠道 | 允许在Screens播放器中显示您的自定义Web应用程序。 |
| 1x1 分屏渠道 | 允许在单个区域中查看组件。 |
| 1x2 分屏渠道 | 允许在两个区域（水平拆分）中查看资源。 |
| 2X1分屏渠道 | 允许查看两个区域（垂直拆分）中的资产。 |
| 2x2 分屏渠道 | 允许在4个区域（在矩阵中水平拆分和垂直拆分）中查看资源。 |
| 2x3 分屏渠道 | 允许查看两个区域（水平拆分）中的资产，其中一个区域大于另一个区域。 |
| 左或右L栏分屏渠道 | 允许内容作者在大小合适的区域中查看不同类型的资产。 |

>[!NOTE]
>
>分屏渠道将显示分成多个区域，以便您可以并排同时播放多种体验。 体验可以是静态资源/文本或嵌入序列。

>[!IMPORTANT]
>
> 创建内容并将其添加到渠道后，下一步是创建一个位置，然后创建一个显示。 此外，您需要将该渠道分配给显示。 请参阅本节末尾的资源以了解更多信息。
...
1. 单击要编辑的渠道。
1. 单击 **属性** 以查看/编辑渠道属性。 通过下列选项卡，可更改选项。
...
1. 选择要编辑的渠道。
1. 单击 **仪表板** 操作栏，以查看仪表板。 此 **渠道信息**，**已分配显示区**、和 **待处理启动项** 面板将打开，如下图所示：
...
1. **将渠道设置为脱机**
   1. 选择渠道并选择 **属性** 操作栏中的
   1. 导航到 **渠道** 制表符并确保取消选中 **开发人员模式（强制渠道联机）** option
   1. 单击 **保存并关闭**
1. **更新离线内容**
   1. 选择渠道并选择 **仪表板** 操作栏中的
   1. 导航到 **渠道信息** 面板并单击 *...*
...
1. 导航到渠道为 **测试项目** > **渠道** > **TestChannel**.
   ...
   单击 **仪表板** 查看播放器的状态。 此 **渠道信息** 面板提供了有关渠道处于在线还是离线状态的信息。
