...
将渠道分配给显示屏时，您可以选择指定显示屏的路径或按上下文解析为实际渠道的渠道的角色。

要使用动态嵌入式序列，请按渠道角色分配 ***渠道***。 渠道角色定义显示的上下文。 该角色由各种操作定位，并独立于履行该角色的实际渠道。 此部分描述了一个按角色定义渠道的用例，以及如何将该内容用于全局渠道。您还可以将角色视为分配的标识符，或渠道的上下文中的别名。

### 使用动态嵌入式序列的优势 {#benefits-of-using-dynamic-embedded-sequences}

将序列渠道放在位置而不是渠道文件夹内的主要好处是允许本地或区域作者编辑与其相关的内容，同时限制其在层次结构中上方编辑渠道。
...
* ***单个序列渠道*** ，位于显示屏中具 *有与动态嵌入式序列组件的* 角色相匹配 **的相应位置中的角&#x200B;*色*。**
...
在开始实现此功能之前，请确保您已准备好开始实现动态嵌入式序列的以下先决条件：

* 创建AEM Screens项目(在此示例中， **Demo**)

* 在“渠道”文件夹下 **创建** “全 **局”** 。

* 将内容添加到 **Global***渠道&#x200B;**(**请检查* Resources.zip，获取相关资产)

下图显示了“全局 **渠道** ”( **Global****** 渠道)文件夹中的Demo项目。
![screen_shot_2018-09-07at21032pm](assets/screen_shot_2018-09-07at21032pm.png)

### 资源 {#resources}

您可以下载以下资源（图像并将这些资源添加到资产中），并进一步将这些资源用作渠道内容以用于演示目的。
...
1. **在每个位置文件夹下创建两个位置和一个渠道。**
   ...
   1. 重复步骤(b)，现在从向导 **中选择序列渠道** 。 对于此 **渠道** ，输入标题 **作为区域A** ，输 **入名称作为** **区域**。
   >[!CAUTION]
   >
   >请确保在创建渠道 **区域A**&#x200B;时，输入标题 **A作为区域** A **,**********&#x200B;并输入NameNameAsRegion。

   ![screen_shot_2018-09-13at22857pm](assets/screen_shot_2018-09-13at22857pm.png)

   同样，在标题为“商店3”和“ **商店4** ”的“ **区域B** ”下创建两个位置 ****。 此外，创建序列 **渠道,** 将“名称 ******”作为“** B区域”，将“名 ********&#x200B;称”作为“区域”。

   >[!CAUTION]
   >
   >请确保对在区域A和区域B中创建的渠道使用同 **一名称** ，以 **便作为区域****使用**。
...
1. **在每个位置下创建显示和渠道。**
   ...
   1. 重复步骤(b)，此时从向导中 **选择序列渠道** 。 输入 **Title** as **Store** 1 **渠道** ,Name as **Store** Reading。
   >[!CAUTION]
   >
   >创建序列渠道时很重要，该渠道的 **Title** （标题）可以作为您的要求，但 **Name** （名称）应在所有本地渠道中相同。
   >
   >在此，A **A A区域和B区域下的渠道与A A A区域名下的** Store 1、Store 2Store **Store 3Store、Store 4Shore的同名Shore** A A RegionB区域下的渠道和Store ******************************** S共享AAS。

   ![screen_shot_2018-09-19at120206pm](assets/screen_shot_2018-09-19at120206pm.png)

   同样，在 **Store 2** (名称为 **Store2Display** )下创建一个显示屏，作为 **Store2Display和渠道Store2******&#x200B;渠道。

   >[!NOTE]
   >
   >请确保对于在商店1和商店2中创建的渠道，您可以 **使用与商店** 1和 **商店2** 相同的 **名称**。

   ![screen_shot_2018-09-19at120329pm](assets/screen_shot_2018-09-19at120329pm.png)

   按照以上步骤，在“区域B”下的“商店3 **”和“商店4”中创建** 渠道并显 **示该******。 同样，请确保在创建 **渠道** Store3 **渠道和** Store4渠道时使用与 ******** store相同的名称。

   下图显示了 **Store 3中的显示和渠道**。

   ![screen_shot_2018-09-19at120448pm](assets/screen_shot_2018-09-19at120448pm.png)

   下图显示了 **Store 4中的显示和**&#x200B;渠道。
...
1. **将内容添加到渠道各自位置中。**
   ...
   同样，导航到 **Demo** -> **Locations** -> **Region B** -> **Region B** -> ClickBard **** Edit action from the action, to drag the assets and drop to your渠道，如下所示：
...
1. **创建计划**
   ...
   在AEM Screens项目中导 **航并选择计划** ，然后单击操作栏中的 **创建** ，以创建新计划。
...
1. **将渠道分配给计划**
   ...
   1. 导航到 **Demo** —> **计划** —> **AdSchedule** （演示），然后单 **** 击操作栏中的仪表板。
   1. 单击 **“已分配渠道** ”面板中的“分配渠道 **”(Assign)，以打开“** 渠道分配”(Assign Assignment **** )对话框。
   1. Select **Reference Channel**.. by path.
   1. 选择“ **渠道路径** ”作 **为Demo** —> ***Global*** ****** Global Path”。
   1. Enter the **Channel Role** as **GlobalAdSegment**.
   1. 选择“ **Initial Load** ”( **初始负载**)、“Idle Screen **”(**&#x200B;空闲屏幕 **)和“User Interaction Supported Hould**”（用户交互支持的Juinital Load）。
   1. 单击&#x200B;**保存**。
   **按角色为区域分配渠道:**
...
1. **将动态嵌入式序列配置为全局渠道。**
   ...
   从其中一个组件中打开属性，然后将 **渠道分配角色输入** RegionAdSegment **中**。

   同样，选择其他组件并打开属性，以将 **渠道分配角色输入****为StoreAdSegment**。
...
1. **将计划分配给每个显示屏**
   ...
   1. 单击 **...** 从“已分 **配渠道和计划** ”面板中，进一步单 **击+分配计划**。
   1. 选择计划的路径(例如，此处， **Demo** —> **计划** —>**AdSchedule**)。
   1. 单击&#x200B;**保存**。

## 查看结果 {#viewing-the-results}

设置好渠道和显示完成后，请启动AEM Screens播放器以视图内容。
...
以下输出将根据显示路径确认AEM Screens播放器中的渠道内容。

**方案1**:

如果将显示路径指定为 **Demo** —> **Locations** —> **Region A —>** Region A —> **Store 1 —****** Display Store1 Store，则后续内容将在您的AEM Screens播放器上显示。
...
您可以创建全局、区域或本地作者来编辑与其相关的内容，同时限制他们在层次结构中的上方编辑渠道。
...
1. **修改每个用户组的权限**
   ...
   **对于全局作者：**
   ...
   1. 导览至 ***/content/screens/demo/locations/region-a并检查所有权限*** 。 同样，请检查 **region-b的权限**。
   请参阅下图以了解这些步骤：
   ![screen_shot_2018-09-18at115752am](assets/screen_shot_2018-09-18at115752am.png)

   下图显示，现在 **Global Access AndGlobal** StoreStoreStoreStoreStoreStoreStoreStoreStore3,StoreStoreStoreA ****************************,StoreBStore3,SeShoreToreASeSerS.S.S.S.S.Shore.S.S.S.S.S.S.S.A.S.A.S.S.S.S.S.S.S.Store.S.StoreS.S.S.Store.A.S.S.S.S.S.S.S.S.S.
   ...
   **对于区域——作者：**
   ...
   下图显示，Region-User现在可以访问 **A** 、 **B区域A和B区域Store** 1Store、 **StoreStore 3、StoreStore 3和Store 4StoreStoreBut不访问GlobalChoupStores的A****************** 。
   ...
   **对于Store-Author:**
   ...
   下图显示， **Store-User******-Store 1.2, **Store** Store 122, **Store 4和** Store只能访问4家商店-Store-User **************** Store 1.3，但目前Store-StoreGlobal或Greation（AdB Region，即BB Region）渠道)的访问权限。
