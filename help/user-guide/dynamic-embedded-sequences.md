...
在为显示分配渠道时，您可以选择指定显示的路径，也可以选择按上下文解析为实际渠道的渠道角色。

要使用动态嵌入式序列，可通过以下方式分配渠道 ***渠道角色***. 渠道角色定义显示的上下文。 该角色由各种操作定向，独立于履行该角色的实际渠道。 本节介绍按角色定义渠道的用例，以及如何将该内容应用于全局渠道。 您还可以将角色视为分配的标识符，或者在上下文中视为渠道的别名。

### 使用动态嵌入式序列的优势 {#benefits-of-using-dynamic-embedded-sequences}

将序列渠道放在位置而非渠道文件夹内的主要好处是，允许本地或区域作者编辑与他们相关的内容。 所有这些信息，同时限制编辑层次结构中较高位置的渠道。

引用 *渠道（按角色）*，可让您创建本地版本的渠道，以动态解析特定于位置的内容，还可让您创建将内容用于特定于位置的渠道的全局渠道。
...
1. **在每个位置文件夹下创建两个位置和一个渠道。**
   ...
   >[!CAUTION]
   >
   >确保在创建渠道时 **区域A**，输入 **标题** 作为 **区域A** 和 **名称** 作为 **区域**.
...
1. **将内容添加到各自位置的渠道中。**
   ...
   导航至 **演示** > **位置** > **区域A** > **区域A** 并选择 **编辑** 从操作栏中。 拖放要添加到渠道的资产。
   ...
   同样，导航到 **演示** > **位置** > **区域B** > **区域B** 并选择 **编辑** 将资产拖放到渠道中，如下所示：
...
1. **创建计划**
   ...
   导航并选择 **时间表** AEM Screens文件夹。 然后选择 **创建** 从操作栏中。
...
1. **将渠道分配给计划**
   ...
   1. 导航到 **演示** > **时间表** > **AdSchedule** 并选择 **仪表板** 从操作栏中。
   1. 选择 **+分配渠道** 从 **已分配渠道** 面板，以便您打开 **渠道分配** 对话框。
   1. 选择 **引用渠道** 按路径。
...
1. **为每次显示分配计划**
   ...
   1. 选择 **仪表板** 从操作栏中。
   1. 在功能板上，选择 **...** 从 **已分配的渠道和计划** 面板，然后选择 **+分配计划**.
...
1. **创建组**
   ...
   1. 选择 **工具** > **安全性** > **组**.
   1. 选择 **创建组** 并输入 **Global-Author** 在 **ID**.
   1. 选择&#x200B;**保存并关闭**。
...
1. **创建用户并将用户添加到组**
   ...
   1. 导航到 **Adobe Experience Manager**.
   1. 选择 **工具** > **安全性** > **用户**.
   1. 选择 **创建用户** 并输入 **全局用户** 在 **ID**.
   1. 输入 **密码** 并确认此用户的密码。
   1. 选择 **组** 选项卡，并在中输入组名称 **选择组**&#x200B;例如，输入 **Global-Author** 添加 **全局用户** 到那个特定的群组。
   1. 选择&#x200B;**保存并关闭**。
...
1. **访问每个组的权限**
   ...
   1. 搜索 **Global-Author** 并选择 **权限** 选项卡，如下图所示。
...
1. **修改每个组的权限**
   ...
   **对于Global-Author：**
   ...
   以下显示了 **全局用户** 有权访问 **全局渠道**，并且两者 **区域A** 和 **区域B** 全部四间店铺之薪酬如下： **商店1**， **商店2**， **商店3**、和 **商店4**.
   ...
   **对于Region-Author ：**
   ...
   以下显示了Region-User对两者的访问权限 **区域A** 和 **区域B**，共拥有四家店铺，即 **商店1**， **商店2**， **商店3**、和 **商店4**，但不访问 **全局** 渠道。
   ...
   **对于Store-Author：**
   ...
   以下显示了 **存储用户** 只能访问 **商店1**， **商店2**， **商店3**、和 **商店4**，但没有访问 **全局** 或区域(**区域A** 和 **区域B**)渠道。
