...
>
>有一些浏览器引擎与不兼容 *`SameSite=None`* 由AEM 6.5.5颁发给AEM 6.5.7的登录令牌中使用的属性。通常，将浏览器升级到最新可用版本即可解决此问题。 有时，可能无法进行此类升级，例如使用智能显示屏、机顶盒或其他具有嵌入式浏览引擎的设备。
...
1. 您应该会看到一个新选项 *`User agents to be exempted from samesite attribute`*. 使用与用户代理对应的正则表达式填充此变量，该用户代理与 *SameSite=None* 属性。
   ...
   >[!NOTE]
   >
   >请参阅 [SameSite=None：已知的不兼容客户端](https://www.chromium.org/updates/same-site/incompatible-clients) 以了解更多详细信息。 对于Tizen播放器，请使用正则表达式： `(.*)Tizen(.*)`.
...
| 云模式 | 如果希望Tizen播放器as a Cloud Service连接到Screens，则设置为true。 设置为false以连接到AMS或本地AEM。 |
