...
1. 点按/单击 **分配渠道** 在操作栏中
   ...
   点按/单击 **仪表板** 并单击 **+指定渠道** 从 **已分配渠道** 面板以打开 **渠道分配** 对话框。
...
利用计划，可在应显示渠道时以文本形式提供描述。 通过它还可以定义开始日期(**保持活动状态起始日期**)和结束日期(**保持活动状态结束日期**)来显示渠道。
...
与合并时的计划 **DayParting**，设置一个全局计划，其中多个渠道在一天中的特定时间运行，然后一次性将该设置重复用于所有显示。

DayParting是指将一天划分为多个时段，并指定在所需时间播放哪些内容。 AEM Screens允许您根据需要按照一天、一周或一个月中的时段划分来安排渠道。

以下示例在三种不同的情景中解释了渠道中的日期划分：

#### 在划分为多个时隙的某天播放内容 {#playing-content-on-a-single-day-divided-into-multiple-time-slots}

此示例显示餐厅如何使用日时段来展示其早餐、午餐和晚餐菜单。

在此，我们将每天划分为三个不同的时隙，以便渠道内容在一天中的指定时间播放：

| **渠道** | **角色** | **优先级** | **计划** |
|---|---|---|---|
| Menu_A | 早餐 |  | 6:00 至 11:00 期间 |
| 菜单B | 午餐 |  | 11:00 至 15:00 期间 |
| Menu_C | 晚餐 |  | 15:00 至 20:00 期间 |

#### 在一周中的特定日期播放内容 {#playing-content-on-a-particular-day-of-the-week}

此示例显示了在赌场中实现的分日活动，该赌场的直播活动在每周末的8:00 pm到10:00 pm之间进行，并且在10:00 pm之后到1:00 am之间可提供晚餐菜单上的特别优惠
...
在这里，您将创建按月进行日期分段，这样渠道内容就可以按照一年中的指定月份播放。
...
>* [处理渠道中资产的重复项](https://docs.adobe.com/content/help/en/experience-manager-screens/user-guide/authoring/product-features/channel-level-activation.html#handling-recurrence-in-assets)
